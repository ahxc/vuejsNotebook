  ...
  computed: {
    totalPrice() {
      ...
      return this.books.length === 0
        ? 0
        : this.books.map(book => book.price * book.count).reduce((preValue, currentVlue) => { return preValue + currentVlue; }, 0);
    }
  },
  methods: {
    increment(index) {
...
//简写
// runduce(函数, 和列表第一个元素进行计算的值)
// let newNum = newNums2.reduce((preValue, currentValue) => {preValue + currentValue}, firstValue)
console.log(newNum);
